...
func NewCowHookMock(address common.Address, client *ethclient.Client) (*CowMainTransactor, error) {
	// Create a contract instance using NewBoundContract
	abiContent, err := os.ReadFile("CowHookMock.abi")
	if err != nil {
		panic(err)
	}
	...
	return &CowMainTransactor{contract: boundContract}, nil
}
